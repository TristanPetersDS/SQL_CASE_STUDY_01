...
SELECT COUNT(*) AS MemberNoCharge
FROM Facilities
WHERE membercost = 0.00;
...
CASE WHEN monthlymaintenance > 100 THEN 'Expensive'
	 WHEN monthlymaintenance <= 100 THEN 'Cheap' END AS costlabel 
FROM Facilities;
...
-- Select bookings which cost more than $30
SELECT f.name, CONCAT(m.firstname, ' ', m.surname) member_name,
	CASE WHEN memid = 0 THEN f.guestcost*slots
...
WHERE DATE(starttime) = '2012-09-14'
	AND (CASE WHEN memid = 0 THEN f.guestcost*slots
		 	  ELSE f.membercost*slots END) > 30
ORDER BY cost DESC;


/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT c.name, CONCAT(m.firstname, ' ', m.surname) member_name, c.cost
FROM Members m 
INNER JOIN (SELECT bookid, memid, f.name, 
				CASE WHEN b.memid = 0 THEN f.guestcost*slots
		 			 ELSE f.membercost*slots END AS cost
				FROM Bookings b
				INNER JOIN Facilities f 
				USING (facid)
				WHERE DATE(starttime) = '2012-09-14') AS c
USING (memid)
WHERE c.cost > 30
